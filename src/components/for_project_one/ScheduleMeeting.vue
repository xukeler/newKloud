<template>
<div class="forProject-schedule-meeting">
  <div class="kloudsyncPaneGutter"></div>
  <div class="kloudsyncCreateMeeting">
      <div class="header">{{$t('ScheduleMeeting.NewMeeting')}}</div>
      <div class="content">
          <Row :gutter="16">
              <Col span="8"><input id="forProject-schedule-slecte-date" style="width:100%" /></Col>
              <Col span="8"><input id="forProject-schedule-start-time" style="width:100%" /></Col>
...
export default {
  name:'forProject-schedule-meeting',
  data() {
    return {
      selectedContacts:[],
      selectedContactsOk:[],
  ...
  mounted() {
    ...
    self.$nextTick(() => {
      util.LoadKendoFiles().then(()=>{
        $("#forProject-schedule-slecte-date").kendoDatePicker({
          value: start,
          change: function() {
            self.meetingTime.date=this.value();
          }
        });
        $("#forProject-schedule-start-time").kendoTimePicker({
          value: start,
          change: function() {
