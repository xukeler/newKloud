...
export default {
    ...
    methods: {
        ...
        async JoinMedia(meetingid,userid,username,role,openVideo,openAudio,callback,retry = 0) 
        {
            ...
            await util.LoadJsFiles(["https://cdn.jsdelivr.net/npm/agora-rtc-sdk@2.8.0/AgoraRTCSDK.min.js"]);
        ...
        Leave() {            
            if(this.isLeaveing==true)return;
            //console.log("Leave");
            this.isLeaveing=true;
            $("#mediaview").hide();
            $("#liveMediaContainer").empty().hide();
        ...
        SwitchDevice(type,deviceId,onSuccess,onFailure){
            ...
            localStream.switchDevice(type,deviceId,onSuccess,onFailure);
        },
        getShareScreenRes() {
            return new Promise((resolve, reject) => {
        ...
        playStreamOnMaxUser(stream) {
            ...
            this.getUsersInfo(userlist).then(userinfo => {
                ...
                this.playStream("mediauserview" + stream.getId(), stream);
        ...
        displayStream(tagId, stream, width, height, className, parentNodeId) {
            ...
            this.playStream(tagId + stream.getId(),  stream);
        ...
        playStream(tagId, stream){
            // var speaker = this.getSpeakerSetting();
            // var setSpeaker=true;
            // if (speaker && speaker != "") {
            //     stream.setAudioOutput(speaker, ()=>{
                    
            //     }, ()=>{
            //         setSpeaker=false;
            //     });
            // }
            stream.play(tagId, {fit: "contain"}, (err)=>{
                if(err==null)
                {
                    this.setStreamSpeaker(stream.getId());
                }
                if (err && err.status !== "aborted" && stream.getId()+""!=uid+""){
                    // 播放失败，一般为浏览器策略阻止。引导用户用手势触发恢复播放。
                    //audio225016485
                    console.error(err);
                    // var audio=$(tagId).find("audio");
                    // var username="";
                    // if(audio.attr("id"))
                    // {
                    //     username=$("#mediauser-"+audio.attr("id").replace("audio","")).find("div.videoface").next().html();
                    // }

                    // audio.css({"z-index":"99999","position":"absolute"});
                    // audio[0].controls=true;                    
                    // this.$Message.error({content: '播放'+username+'的声音失败,请手动点击播放按钮!',duration: 12,closable:true});
                    stream.resume().then(function (result) {
                    ...
                    this.$Modal.confirm({
                        zIndex:99999,
                        render: h => {
                            return h("Div", {domProps: {innerHTML: "播放"+username+"声音失败,要重试吗?"}});
                        },
                        onOk: (evt) => {
                            stream.resume().then(function (result) {
                                console.log('恢复成功：' + result);
            ...
            this.getUsersInfo(userlist).then(userinfo => {
                ...
                if (
                    ...
                    if (false) {
                        ...
                        $("#mediamaxuser").append(html);
                        $("#mediavideoback").bind("click", this.backToLesson);
                        if (firstStream.isPlaying()) firstStream.stop();
                        this.createUserContainer4Max(
                            firstStream.getId(),
                        ...
                        this.playStream("mediauserview" + firstStream.getId(),firstStream);
                ...
                if (localStream != null && !isLocalStreamShow) {
                    ...
                    this.playStream("mediauserview" + localStream.getId(),localStream);
                ...
                for (var i = 0; i < remoteStreamList.length; i++) {
                    ...
                    if (remoteStreamList[i].stream.isPlaying()) remoteStreamList[i].stream.stop();
                    this.createUserContainer4Max(
                        remoteStreamList[i].stream.getId(),
                        this.getUserName(
                            orderedInfo,
                    ...
                    this.playStream("mediauserview" + remoteStreamList[i].stream.getId(),remoteStreamList[i].stream);
        ...
        showAllStream4FullScreen(size) {
            ...
            this.getUsersInfo(userlist).then(userinfo => {
                ...
                for (var i = 0; i < remoteStreamList.length; i++) {
                    ...
                    if (remoteStreamList[i].isPlaying()) remoteStreamList[i].stop();
                    this.createUserContainer4Max(
                        remoteStreamList[i].stream.getId(),
                        this.getUserName(
                            orderedInfo,
        ...
        showScreenShare(stm) {
            ...
            if (stm.isPlaying()) stm.stop();
            this.playStream("screenshare_" + stm.getId(),stm);
        ...
        ResetSpeaker(id) {
            var speaker = id || this.getSpeakerSetting();
            if (speaker && speaker != "") {
                $("#media_wrapper,#liveMediaContainer").find("audio").each(function(i) {
                        //$(this)[0].setSinkId(speaker);
                        document.getElementById($(this).attr("id")).setSinkId(speaker);
                    });
            }
        },
        subscribeStreamEvents() {
            client.on("stream-added", evt => {
                ...
                if(parseInt(sid)>1000000000 && parseInt(sid)-1000000000==parseInt(uid))
                {
                    return;
                }
