...
export default {
  ...
  methods: {
    ...
    handlePropertySave() {
      if(this.catalogName == ""){
        this.$Notice.open({title: this.$t('ChatFloating.NotificationTitle'),desc: this.catalogTypeName + this.$t('KloudSyncDocLeftPart.AlertNameCannotBeEmpty'), top: 24,duration: 4, left:true });
        return;
      }
    ...
    handleDeleteTopicOK() {
      if(!this.mergeTopic || this.mergeTopic <= 0){
        this.$Notice.open({title: this.$t('ChatFloating.NotificationTitle'),desc: "Please select a SyncRoom to merge.", top: 24,duration: 4, left:true });
        return;
      }
